...
14. **html2canvas no captura elementos `position:fixed` off-screen** (`left:-9999px`). Con `fixed`, el browser posiciona el elemento relativo al viewport → queda fuera del área de captura → canvas vacío → PDF vacío (~3KB en lugar de >50KB). Fix: usar `position:absolute; left:-9999px` en el contenedor de render oculto (ej. `#pdf-render-area`). Aplica a cualquier patrón de render-offscreen para html2pdf.js.
15. **Verificá siempre el formato real de imágenes en `assets/` con `file <path>`** — no confiar en la extensión. WebP con extensión `.png` se sirve con `Content-Type: image/png` (incorrecto) → `cargarLogoBase64()` produce `data:image/png;base64,[bytes WebP]` → MIME mismatch → Safari rechaza la imagen en el contexto SVG foreignObject de html2canvas → todo el render del canvas falla silenciosamente → PDF completamente en blanco. Fix: convertir a PNG real con Pillow (`Image.open().convert('RGB').save(..., 'PNG')`). Además: `useCORS:true` + `allowTaint:true` juntos son contradictorios en html2canvas — usar solo `useCORS:true`.
...
- 🔄 Bloque J: Invoicing — Etapa 1 ✅ VALIDADA · Etapa 2 ✅ CODEADA (schema v2, `e448235`) · Etapa 3 (pagos manuales) ✅ CODEADA · pendientes: Fase 3 (auto-populate líneas desde OS) y Fase 4B (Send to Client / SendGrid)
